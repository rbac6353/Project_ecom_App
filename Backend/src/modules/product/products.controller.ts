...
import { AskProductDto } from '@modules/ai/dto/ask-product.dto';
...
export class ProductsController {
  ...
  findAll(
    ...
    @Query('subcategoryId') subcategoryId?: string, // ✅ เพิ่ม subcategoryId (แนะนำให้ใช้)
    @Query('subcategory') subcategory?: string, // ✅ เก็บไว้เพื่อ backward compatibility
    @Query('shopType') shopType?: string, // ✅ mall = สินค้าจากร้าน Mall เท่านั้น
  ) {
    // ส่งเป็น object ไปให้ service จัดการ
    return this.productsService.findAll({
      categoryId,
      subcategoryId, // ✅ ส่ง subcategoryId (แนะนำ)
      subcategory, // ✅ ส่ง subcategory name (backward compatibility)
      page,
      limit,
      keyword,
      sortBy,
  ...
  // ✅ Suggestion Endpoint สำหรับ Search Autocomplete
  @SkipThrottle()
  @Get('suggestions')
  getSuggestions(@Query('keyword') keyword: string) {
    return this.productsService.getSuggestions(keyword);
  }
  ...
  // ✅ Admin API: Backfill slug สำหรับสินค้าที่ slug เป็น NULL (SEO)
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Post('admin/backfill-slugs')
  async backfillProductSlugs() {
    return this.productsService.backfillProductSlugs();
  }

  @UseGuards(JwtAuthGuard)
  @Post()
  @UseInterceptors(AnyFilesInterceptor(fileUploadOptions)) // ✅ File upload with validation (size, mime type)
  ...
  @SkipThrottle()
  @Post(':id/ask-ai')
  async askProductAi(@Param('id') id: string, @Body() dto: AskProductDto) {
    ...
    const answer = await this.aiService.askAboutProduct(
      {
        id: product.id,
        title: product.title,
  ...
  findOne(@Param('id') id: string) {
    ...
    // ✅ Validation: ตรวจสอบว่า id เป็นตัวเลข
    const numericId = Number(id);
    if (isNaN(numericId) || numericId <= 0 || !Number.isInteger(numericId)) {
      throw new BadRequestException('Invalid product ID: must be a positive integer');
    }
