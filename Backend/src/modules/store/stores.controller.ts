...
import { JwtAuthGuard, OptionalJwtAuthGuard, RolesGuard } from '@core/auth';
...
export class StoresController {
  ...
  // ✅ ดึงร้านค้าของฉัน (Seller) - ต้องอยู่ก่อน route :id
  @UseGuards(JwtAuthGuard)
  @Get('my')
  async getMyStore(@Request() req) {
    console.log('GET /stores/my called for user:', req.user.id);
    const store = await this.storesService.findByOwnerId(req.user.id);
    // คืนค่าเป็น Array เพื่อให้ตรงกับที่ Frontend คาดหวัง (หรือจะแก้ frontend ก็ได้ แต่ backend ง่ายกว่า)
    // Frontend code: const storesList = Array.isArray(res) ? res : ...;
    // แต่ปกติ findOne จะคืน object เดียว
    // ลองดู code frontend: res = await client.get(`/stores/my`);
    // const storesList = Array.isArray(res) ? res : ...;
    // if (storesList.length > 0) ...

    // ดังนั้นเราควรคืนเป็น Array 1 ตัว ถ้าเจอ, หรือ Empty Array ถ้าไม่เจอ
    return store ? [store] : [];
  }
  ...
  async updateStore(@Request() req, @Body() body: any, @UploadedFile() file: any) {
    ...
    } else if (body.logoBase64 && String(body.logoBase64).startsWith('data:image')) {
      try {
        console.log('📷 Uploading store logo from base64...');
  ...
  async getStoreProfile(@Param('id') id: string, @Request() req) {
    // ถ้ามี user จาก token ใช้ userId, ถ้าไม่มีก็ส่ง undefined
    const userId = req.user?.id;
    return this.storesService.getStoreProfile(+id, userId);
  }
