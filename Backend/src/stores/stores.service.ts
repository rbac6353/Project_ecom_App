...
export class StoresService {
  ...
  // ✅ ดึงร้านค้า Mall ทั้งหมด (Public)
  async getMallStores() {
  ...
  async findOneWithDetails(id: number) {
    const store = await this.storesRepo.findOne({
      ...
      order: {
        products: { createdAt: 'DESC' }, // เรียงสินค้าใหม่สุดขึ้นก่อน
      },
    });

    if (!store) throw new NotFoundException('Store not found');

    console.log(`[AdminStoreDetail] Store ID: ${id}, Products Count: ${store.products?.length || 0}`);
    if (store.products && store.products.length > 0) {
      console.log(`[AdminStoreDetail] First Product:`, store.products[0]);
    }
  ...
  async getStoreProfile(storeId: number, currentUserId?: number, includeInactive: boolean = false) {
    ...
    if (includeInactive) {
      console.log(`[DEBUG_STORE] Request Inactive: CurrentUser=${currentUserId}, StoreOwner=${store.ownerId}`);

      // แปลงเป็น Number เพื่อความชัวร์
      const userIdNum = Number(currentUserId);
      const ownerIdNum = Number(store.ownerId);

      if (!currentUserId || userIdNum !== ownerIdNum) {
        console.log(`[DEBUG_STORE] Access Denied: User ${userIdNum} is not owner of Store ${store.id} (Owner: ${ownerIdNum})`);
        includeInactive = false; // ปิด flag ถ้าไม่ใช่เจ้าของ
      } else {
        console.log(`[DEBUG_STORE] Access Granted: User is owner`);
      }
    }

    // 2. ดึงสินค้าในร้าน (เอามาสัก 20 ชิ้นล่าสุด) - filter เฉพาะสินค้าที่ active (ยกเว้น seller ดูเอง)
    ...
    console.log(`[DEBUG_STORE] Product Query: Where=${JSON.stringify(whereCondition)}`);
    ...
    console.log(`[DEBUG_STORE] Found Products: ${products.length}`);
